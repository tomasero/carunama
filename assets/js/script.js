...
$(document).ready(function() {
  ...
  $('#copy_button').click(copyButton);
  $('#email_button').click(emailButton);
  $('#select_language').change(updateCountry);
  $('#transfer_button').click(function () {
    console.log('transfer');
    $('#edit_area').text($('#final_span').text())
    $('#edit_area').val($('#final_span').text())
  });

  function translate(text) {
    socket.emit('dream', text);    
  }
  $('#translate_button').click(function () {
    ...
    console.log(txt);
    console.log(val)
    $('#images').html('<div class="col"></div>');
    translate(val);
  });
  //var ctx = document.getElementById('images').getContext('2d');
  var cnt = 0;
  ...
  var imgWidth = 100;
  socket.on("done", function(info) {
    //width = String($('#images > col').length * $('.col').width()) + 'px'
    width = String(info.num * imgWidth) + 'px'
